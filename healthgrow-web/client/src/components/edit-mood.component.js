...
class EditMood extends Component {
  ...
  componentDidMount() {
    axios.get(`${hostname}/api/moods/`+this.props.match.params.id)
      .then(response => {
        this.setState({
  ...
  onSubmit(e) {
    ...
    axios.post(`${hostname}/api/moods/update/`+this.props.match.params.id, mood).then(function(res)
        {
  ...
  render() {
    ...
    return (
        <div>
      <h3>u actually can't edit moods sry</h3>
        </div>)
...
export default connect(mapStateToProps, mapDispatchToProps)(EditMood);
